...
description: "Use for TypeScript/NestJS unit testing with Jest, @golevelup/ts-jest, and in-memory databases. Trigger when setting up Jest, configuring test infrastructure, writing or adding .spec.ts tests, testing services/use cases/controllers/guards/interceptors/pipes/filters, mocking dependencies with createMock or DeepMocked, improving coverage, reviewing test quality, running or analyzing unit tests, fixing failing/flaky/slow tests, debugging setup/mock/timeouts/open handles, optimizing test organization/performance, or doing TDD."
---
